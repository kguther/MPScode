...
void overlap::loadMPS(mps const*const psiIn, mps const*const phiIn){
  ...
  Lctr=new mpsEntryType[L*D*D];
  Rctr=new mpsEntryType[L*D*D];
  //Initialize F-Matrix
...
void overlap::subContractionStartLeft(mpsEntryType *&pStart, int i){
  pStart=Lctr+i*D*D;
}

//---------------------------------------------------------------------------------------------------//

void overlap::subContractionStartRight(mpsEntryType *&pStart, int i){
  pStart=Rctr+i*D*D;
}
...
void overlap::calcCtrIterLeft(int i){
  ...
  lDR=phi->locDimR(i);
  ld=phi->locd(i);
  tmpContainer<mpsEntryType > innerContainer(1,ld,lDR,lDL);
  mpsEntryType simpleContainer;
  mpsEntryType *source, *target;
  if(i>0){
...
void overlap::calcCtrIterLeftQNOpt(int i, mpsEntryType const*const source, mpsEntryType *const target){
  int lDL=phi->locDimL(i);
  int ld=phi->locd(i);
  int lDR=phi->locDimR(i);
  siteQNOrderMatrix localIndexTable=phi->indexTable().getLocalIndexTable(i);
  mpsEntryType const *localMatrix;
  tmpContainer<mpsEntryType > innerContainer(1,ld,lDR,lDL);
  for(int si=0;si<ld;++si){
    for(int ai=0;ai<lDR;++ai){
      for(int aim=0;aim<lDL;++aim){
...
void overlap::calcCtrIterRightQNOpt(int i, mpsEntryType const*const source, mpsEntryType *const target){
...
void overlap::updateF(int i){
  ...
  mpsEntryType simpleContainer;
  mpsEntryType zone=1.0;
  mpsEntryType *leftPart, *rightPart;
  mpsEntryType const *localMatrix;
  if(i>0){
...
mpsEntryType overlap::fullOverlap(){
  return Rctr[0];
}

//---------------------------------------------------------------------------------------------------//

mpsEntryType overlap::getFullOverlap(){
  for(int i=L-1;i>=0;--i){
    calcCtrIterRight(i);
  }
  return Rctr[0];
...
mpsEntryType overlap::applyF(mpsEntryType *vec, int i){
